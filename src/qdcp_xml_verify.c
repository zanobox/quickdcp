...
int xmlsec_verify_init() {
    ...
    /* init xmlsec lib */
    if (xmlSecInit() < 0) {
        //~ QDCP_LOG(LOG_ERROR, "xmlsec initialization failed");
        return(QDCP_FAILURE);
    }

    /* Check loaded library version */
    if (xmlSecCheckVersion() != 1) {
        //~ QDCP_LOG(LOG_ERROR, "loaded xmlsec library version is not compatible");
        return(QDCP_FAILURE);
    }

    /* Init crypto library */
    if (xmlSecCryptoAppInit(NULL) < 0) {
        //~ QDCP_LOG(LOG_ERROR, "crypto initialization failed");
        return(QDCP_FAILURE);
    }

    /* Init xmlsec-crypto library */
    if (xmlSecCryptoInit() < 0) {
        //~ QDCP_LOG(LOG_ERROR, "xmlsec-crypto initialization failed");
        return(QDCP_FAILURE);
    }
...
int xml_verify(char *filename) {
    ...
    if (doc == NULL) {
        //~ QDCP_LOG(LOG_ERROR, "unable to parse file %s", filename);
        goto done;
    }
    ...
    if (root_node == NULL) {
        //~ QDCP_LOG(LOG_ERROR, "unable to find root node");
        goto done;
    }
    ...
    if (sign_node == NULL) {
        //~ QDCP_LOG(LOG_ERROR, "signature node not found");
        goto done;
    }
    ...
    if (key_manager == NULL) {
        //~ QDCP_LOG(LOG_ERROR, "create key manager failed");
        goto done;
    }
    ...
    while (x509d_node = xmlSecFindNode(cur_node, xmlSecNodeX509Data, xmlSecDSigNs)) {
        ...
        if (cert_node == NULL) {
            //~ QDCP_LOG(LOG_ERROR, "X509certficate node not found");
            goto done;
        }
        ...
        if (xmlSecCryptoAppKeysMngrCertLoadMemory(key_manager, cert, cert_l, xmlSecKeyDataFormatPem, xmlSecKeyDataTypeTrusted) < 0) {
            //~ QDCP_LOG(LOG_ERROR, "could read X509certificate node value");
            goto done;
        }
    ...
    if (dsig_ctx == NULL) {
        //~ QDCP_LOG(LOG_ERROR, "create signature opendcp failed");
        goto done;
    }

    /* sign the template */
    if (xmlSecDSigCtxVerify(dsig_ctx, sign_node) < 0) {
        //~ QDCP_LOG(LOG_ERROR, "signature verify failed");
        goto done;
    }

    if (dsig_ctx->status != xmlSecDSigStatusSucceeded) {
        //~ QDCP_LOG(LOG_ERROR, "signature validation failed");
        goto done;
    }
